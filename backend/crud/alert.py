...
def upsert_alert(
    ...
    *,
    ...
    try:
        ...
        client.update(index=INDEX_ALERTS, id=_id, body={"doc": body, "doc_as_upsert": True}, refresh=True)
    except Exception:
        # Fallback index
        body["created_at"] = now
...
def list_alerts(
    ...
    *,
    ...
    try:
        res = client.search(index=INDEX_ALERTS, body={"query": query, "sort": [{"created_at": {"order": "desc"}}]}, from_=skip, size=limit, track_total_hits=True)
...
def generate_alerts_for_vulnerability(client: OpenSearch, vuln_doc: dict) -> int:
    ...
    if not configs:
        return 0
    # For each CPE match, collect product tokens
    tokens: set[str] = set()
    ...
    # Query inventories by wildcard on software_name
    created = 0
    for tok in list(tokens)[:20]:  # limit breadth
        try:
            res = client.search(
                ...
                body={
                    "query": {
                        "wildcard": {"software_name.keyword": {"value": f"*{tok}*"}}
                    },
                    "size": 500,
                },
            )

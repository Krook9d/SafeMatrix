...
@router.get("/hosts/", response_model=List[schemas_host.Host])
def list_hosts(
    ...
    """
    hosts = crud_host.get_hosts(client=client, skip=skip, limit=limit)
    return hosts


@router.get("/hosts/{host_id}", response_model=schemas_host.Host)
def read_host(
    *,
    client: OpenSearch = Depends(get_opensearch_client),
    host_id: str,
    ...
    """
    host = crud_host.get_host(client=client, host_id=host_id)
    if host is None:
        raise HTTPException(status_code=404, detail="Host not found")
    return host


@router.put("/hosts/{host_id}", response_model=schemas_host.Host)
def update_existing_host(
    *,
    ...
    """
    updated_host = crud_host.update_host(client=client, host_id=host_id, host_in=host_in)
...
def delete_existing_host(
    *,
    ...
    """
    deleted = crud_host.delete_host(client=client, host_id=host_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Host not found")
    return None # 204 No Content response


@router.post("/hosts/", response_model=schemas_host.Host, status_code=201)
def create_new_host(
    *,
    client: OpenSearch = Depends(get_opensearch_client),
    host_in: schemas_host.HostCreate,

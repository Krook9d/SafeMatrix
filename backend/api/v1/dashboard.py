...
from ...core.opensearch_client import (
    INDEX_HOSTS,
    INDEX_INVENTORIES,
...
def _compute_vulnerability_stats(client: OpenSearch) -> Dict[str, Any]:
    """Query OpenSearch to compute vulnerability statistics."""
    ...
    total_vulnerabilities = (
        vulnerabilities_response.get("hits", {}).get("total", {}).get("value", 0)
    )
    severity_buckets = (
        vulnerabilities_response.get("aggregations", {})
    ...
    critical_vulnerabilities = next(
        (b["doc_count"] for b in severity_buckets if b["key"] == "CRITICAL"), 0
    )

    recent_vulnerabilities = []
    for hit in vulnerabilities_response.get("hits", {}).get("hits", []):
        ...
        recent_vulnerabilities.append(
            {
                "cve_id": vuln.get("id", ""),
                "description": (
                    vuln.get("descriptions", [{}])[0].get("value", "")
...
def get_dashboard_stats(
    ...
    hosts_response = client.search(
        ...
        body={
            "size": 0,
            "track_total_hits": True,
            "aggs": {"os_distribution": {"terms": {"field": "os.name", "size": 10}}},
        },
    )
    ...
    inventories_response = client.search(
        ...
        body={
            "size": 0,
            "track_total_hits": True,
            "aggs": {"unique_hosts": {"cardinality": {"field": "host_id"}}},
        },
    )
    ...
    total_software = (
        inventories_response.get("hits", {}).get("total", {}).get("value", 0)
    )

    # OS distribution
    os_buckets = (
        hosts_response.get("aggregations", {})
        .get("os_distribution", {})
        .get("buckets", [])
    )
    ...
    if include_vuln:
        vuln_stats = _compute_vulnerability_stats(client)
        data.update(vuln_stats)

    return data


@router.get("/dashboard/vulnerability-stats")
def get_vulnerability_stats(
    client: OpenSearch = Depends(get_opensearch_client),
    current_user: schemas_user.User = Depends(get_current_user),
) -> Dict[str, Any]:
    """Return vulnerability statistics separately."""

    return _compute_vulnerability_stats(client)
